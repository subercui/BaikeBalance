...
class Model:
	...
	#results.shape?40*1
        ...
        def step(idx,*states):
            ...
            new_states=self.model.forward(idx,prev_hiddens = states)
            return new_states#不论recursive与否，会全部输出
        
        x = self.x
        outputs_info =[dict(initial=layer.initial_hidden_state, taps=[-1]) for layer in self.model.layers if hasattr(layer, 'initial_hidden_state')]
